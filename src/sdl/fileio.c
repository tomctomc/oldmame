...
#define MAXPATHC 64 /* at most 64 path entries */
#define MAXPATHL 4096 /* at most 4096 character path length */
...
void *osd_fopen(const char *game,const char *filename,int filetype,int write)
{
	...
	FakeFileHandle *f;
	//printf( "TOMCXXX: open(%s,%s,%d,%d)\n", game, filename, filetype, write);
...
int osd_fread(void *file,void *buffer,int length)
{
	...
	int read = 0;
	//printf( "TOMCXXX: osd_fread(%p,%p,%d)\n", file, buffer, length);
