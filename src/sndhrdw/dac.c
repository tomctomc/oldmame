...
static FILE *TOMCXXXWAVDATFILE = NULL;
static int TOMCXXXWAVDATCOUNT  = 0;
void DAC_data_w(int num , int data)
{
#ifdef TOMCXXXX
	if( !TOMCXXXWAVDATFILE ) {
		TOMCXXXWAVDATFILE = fopen("/tmp/wav_from_dac.txt", "wb");
	}
	TOMCXXXWAVDATCOUNT++;
	if( TOMCXXXWAVDATCOUNT % 32 == 0 ) {
		fprintf(TOMCXXXWAVDATFILE,"%3d,\n",data);
		TOMCXXXWAVDATCOUNT=0;
	}
	else {
		fprintf(TOMCXXXWAVDATFILE,"%3d, ",data);
	}
#endif /* TOMCXXXX */
	//printf( "TOMCXXX: DAC[%8d] data {%8d} amp=%8d -> ", num, data, amplitude_DAC[num]);
	//if( data == amplitude_DAC[num] ) { printf( "(same)\n"); } //TOMCXXXX
	/* check current data */
